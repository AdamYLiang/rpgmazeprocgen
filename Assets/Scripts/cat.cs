...
public class cat : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		if(gameManager.listOfSlimes.Count == 0){
			return;
		}

		else{

			for(int i = 0; i < gameManager.listOfSlimes.Count; i++){

				Vector3 directionToMouse = gameManager.listOfSlimes[i].transform.position - transform.position;
					...
					if(Physics.Raycast(catRay, out catRayHitInfo, 10f)){
						if(catRayHitInfo.collider.tag == "Mouse"){
							...
							if(catRayHitInfo.distance <= 3f){
								...
								Destroy(gameManager.listOfSlimes[i].gameObject);
							}
							else{
